...
| The collapsed rail is an attribute on `<html>`, flipped by document-level JS | Same constraint as the drawer: the nav is in a static layout, so collapsing cannot be Blazor state. The platform has no collapse primitive, so a small delegated script owns it, remembers it in `localStorage`, and ZenSideNav restores it inline before first paint. `<html>` rather than the `<aside>` because it is the one element that script can reach before the rail exists, and enhanced navigation resets it exactly as it resets the theme — one `enhancedload` fix covers both. The tooltip is a top-layer popover because the scrolling rail clips sideways. |
